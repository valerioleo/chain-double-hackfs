...
export const sendTransaction = async (chainId, contractInstance, value, method, ...args) => {
  try {
    const {defaultAccount: from} = await getDefaultAccount({chainId});
    ...
    const gasLimit = await estimateGas(value).apply(null, gasArgs);

    const methodResult = contractInstance
      .methods[method](...methodArgs).send({from, gas: gasLimit, value});
