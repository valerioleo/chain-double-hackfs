...
export const estimateGas = value => async (method?, from?, ...params) => {
  const loadedMeethod = await method(...params);
  const estimation = await loadedMeethod.estimateGas({from, value});
...
export const signAndSendTxRoot = waitForReceipt => async (
  ...
  try {
    ...
    const gasLimit = await estimateGas(value)(method, from, ...args);
...
export const getTransactionData = async (providerOptions, {
  ...
  try {
    ...
    const gasLimit = method === 'constructor'
      ...
      : await estimateGas(value)(contractInstance.methods[method], from, ...args);
